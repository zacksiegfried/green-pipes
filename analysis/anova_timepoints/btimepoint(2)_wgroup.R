...
for (df in 1:length(csv_df_list)){
  ...
  biomarker_formula <- as.formula(paste0(biomarker_name, " ~ time"))
  ...
  group_mods_res <- group_mods %>%
    mutate(# Get Pairwise Comparisons across groups within each time point
      means= map(fit, ~pairs(emmeans::emmeans(.x,~time,data=.x$data))),
  ...
  p_val_group_list[[df]] <- group_mods_res %>% unnest(emmeans_tidy) %>%
    dplyr::select(group, term, contrast, estimate, std.error, p.value) %>%
  ...
  anova_group_list[[df]] <- group_mods_res %>% unnest(aov_tidy) %>%
    dplyr::select(group, term, statistic, df, p.value) %>%
    mutate(p.adj = ifelse(p.value < 0.001, "p<0.001", paste0("", format(round(p.value, 3), nsmall=3))), biomarker = biomarker_name)
}
...
group_p_merged_df <- p_val_group_list %>%
  reduce(full_join) %>%
  dplyr::select(-c("term"))
write.csv(group_p_merged_df, paste0(parent_dir, "/1tables/time_pvals.csv"), row.names = FALSE)

group_p_merged_df <- anova_group_list %>%
  reduce(full_join)
write.csv(group_p_merged_df, paste0(parent_dir, "/1tables/time_anova.csv"), row.names = FALSE)
